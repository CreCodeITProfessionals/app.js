...
var key = fs.readFileSync(__dirname + '/certificates/server.key');
var cert = fs.readFileSync(__dirname + '/certificates/server.crt');
var options = {
  key: key,
  cert: cert
};
...
status [True, False],
role [1, 0]
t_psss [Teacher/Admin Password]
s_pass [Student/User Password]
created_date
*/
...
app.post('/api/room/delete', function (req, res) {
    if (req.body.room_id === undefined || req.body.room_id === '') {
        res.status(400).send({ response: false, message: 'Invalid parameters sent in request'});
        return false;
    }
    if (class_rooms_list[req.body.room_id]){
        delete class_rooms_list[req.body.room_id];
        res.status(200).send({ response: true, message: 'Room delete successfuly' });
    }else{
        res.status(400).send({ response: false, message: 'No such room exist'});
    }
});

// Get Room
app.post('/api/room/details', function (req, res) {
    if (req.body.room_id === undefined || req.body.room_id === '') {
        res.status(400).send({ response: false, message: 'Invalid parameters sent in request', data: {} });
        return false;
    }
    if (class_rooms_list[req.body.room_id]) {
        let room = class_rooms_list[req.body.room_id];
        // Send Response
        let room_info = {
        ...
        res.status(200).send({ response: true, message: 'Room info received successfuly', data: room_info });
    } else {
        res.status(400).send({ response: false, message: 'No such room exist', data: {} });
    }
});
...
io.on('connection', function (socket) {
    ...
    socket.on('message', function (message) {
        ...
        switch (message.event) {
            case 'joinRoom':
                try {
                    let is_room_found = false;
                    for(let i in class_rooms_list){
                        // Try Finding Room
                        if(i==message.roomName){
                            is_room_found = true;
                            console.log('Room Found');
                            // If Teaacher
                            if (class_rooms_list[i].t_pass == message.roomKey){
                                class_rooms_list[i].status = true;
                                joinRoom(socket, message.userName, message.roomName, message.chat, 'Teacher', err => {
                                    if (err) {
                                        console.log(err);
                                    }
                                });
                            ...
                            // If Student
                            else if (class_rooms_list[i].s_pass == message.roomKey){
                                if (class_rooms_list[i].status===false){
                                    // Room Not startd yet
                                    socket.emit('message', {
                                        event: 'room-join-events',
                                        type: 'room-not-started',
                                        text: 'Room not started yet. Try later!'
                                    });
                                }else{
                                    joinRoom(socket, message.userName, message.roomName, message.chat, 'Student', err => {
                                        if (err) {
                                            console.log(err);
                                        }
                                    });
                                ...
                                socket.emit('message', {
                                    event: 'room-join-events',
                                    type: 'room-invalid-key',
                                    text: 'Room key invalid'
                                });
                            }
                            break;
                        }
            ...
            case 'end-class':
                ...
                if (class_rooms_list[message.room]){
                    delete class_rooms_list[message.room];
                }
                break;
...
// signaling functions
function joinRoom(socket, username, roomname, chatPermission, role, callback) {
...
http.listen(3000, function () {
    console.log('Online Meeting App on port 3000!');
});